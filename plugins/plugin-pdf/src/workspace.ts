/**
 * Workspace-path discipline shared by every PDF tool.
 *
 ...
 * the workspace.
 *
 * @module @dsh-app/plugin-pdf/workspace
 */
...
export function workspaceRootOf(exec: ToolRunContext): { root: string } | { reason: string } {
  ...
  if (typeof cwd !== 'string' || cwd === '') {
    return { reason: 'A sessão atual não está vinculada a uma área de trabalho; use as ferramentas PDF em uma sessão com uma área de trabalho aberta' }
  }
  return { root: cwd }
}
